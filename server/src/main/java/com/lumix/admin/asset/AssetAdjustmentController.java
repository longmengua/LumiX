...
public class AssetAdjustmentController {
    private final AssetAdjustmentService service; private final AssetAdjustmentAssetConfigurationService assetConfigurationService; private final JdbcTemplate jdbc;
    public AssetAdjustmentController(AssetAdjustmentService service, AssetAdjustmentAssetConfigurationService assetConfigurationService, JdbcTemplate jdbc) { this.service=service; this.assetConfigurationService=assetConfigurationService; this.jdbc=jdbc; }
    ...
    /** 可調整資產由新 adjustment boundary 提供，避免 UI 繼續依賴已移除的 airdrop API。 */
    @GetMapping("/configuration") public ResponseEntity<ConfigurationResponse> configuration(@RequestAttribute(ApiAuthenticationFilter.AUTHENTICATED_USER_ATTRIBUTE) AuthenticatedUser actor) { return ResponseEntity.ok().header(HttpHeaders.CACHE_CONTROL,"no-store").body(new ConfigurationResponse(assetConfigurationService.listActiveSpotAssets(actor))); }
    ...
    public record ConfigurationResponse(List<AssetAdjustmentAssetOption> assets) { }
