...
function latestSessionFile() {
  ...
  if (candidates.length === 0) {
    throw new Error(`No Codex session jsonl found under ${sessionsDir}`);
  }
  return candidates
...
function printSnapshot(title, snapshot) {
  ...
  if (n.fiveHourUsedPercent !== null) {
    console.log(`5h limit: ${n.fiveHourUsedPercent.toFixed(1)}% used / ${(100 - n.fiveHourUsedPercent).toFixed(1)}% left`);
  }
  if (n.weeklyUsedPercent !== null) {
    console.log(`weekly limit: ${n.weeklyUsedPercent.toFixed(1)}% used / ${(100 - n.weeklyUsedPercent).toFixed(1)}% left`);
  }
}
...
function printDelta(start, end) {
  ...
  if (a.fiveHourUsedPercent !== null && b.fiveHourUsedPercent !== null) {
    console.log(`5h limit used delta: ${signed((b.fiveHourUsedPercent - a.fiveHourUsedPercent).toFixed(1))} pp`);
  }
  if (a.weeklyUsedPercent !== null && b.weeklyUsedPercent !== null) {
    console.log(`weekly limit used delta: ${signed((b.weeklyUsedPercent - a.weeklyUsedPercent).toFixed(1))} pp`);
  }
}
...
try {
  ...
  if (command === "status") {
    ...
    if (!fs.existsSync(statePath)) {
      throw new Error(`No baseline found for label "${label}". Run: shells/codex-usage.sh start ${label}`);
    }
    const start = JSON.parse(fs.readFileSync(statePath, "utf8"));
    printDelta(start, latest);
  } else {
