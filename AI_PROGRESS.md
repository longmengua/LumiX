...
P29-R01 API Runtime Infrastructure Topology Foundation: COMPLETED_FOR_INFRASTRUCTURE_FOUNDATION by explicit human reprioritization; Docker Compose、PostgreSQL primary/read-routing configuration、Redis standalone/cluster connection topology are implemented and verified. This does not enable auth, rate-limit, public business API, money movement or production launch.
P29-R02 Persistent User Authentication Runtime: COMPLETED_FOR_PERSISTENT_AUTH_RUNTIME_FOUNDATION; PostgreSQL credential/session、HttpOnly Cookie login/logout/password-change、前端同源 API adapter 與 fail-closed SMTP reset boundary are implemented and Compose integration verified. HUMAN_REVIEW_REQUIRED. SMTP provider/secret/TLS ingress、email verification/MFA、rate-limit/abuse protection、security review and production account launch remain incomplete.
P29-R03 API Authentication and Transport Security Gate: COMPLETED_FOR_SECURITY_GATE_FOUNDATION; global `/api/v1/**` deny-by-default session authentication, HTTPS fail-closed deployment switch and no-store/security response headers are implemented and Compose verified. HUMAN_REVIEW_REQUIRED. TLS certificate/ingress, HSTS, CSRF, rate-limit, security review and production launch remain incomplete.
P29-R04 Authenticated Login History API: COMPLETED_FOR_AUTHENTICATED_READ_FOUNDATION; current-user-only successful session history is exposed through a bounded private API and wired into the account login-history view. P29-R08 extends new records with self-only IP/device snapshots. HUMAN_REVIEW_REQUIRED. It does not provide full security-event audit, MFA, session management UI, rate limiting or production launch readiness.
P29-R05 Loopback Password Reset Delivery: COMPLETED_FOR_LOOPBACK_DEVELOPMENT_RUNTIME; explicit loopback-only HTTP reset-link validation, local Gmail SMTP configuration and forgot-password delivery acceptance are verified. HUMAN_REVIEW_REQUIRED. Any non-loopback HTTP URL remains rejected; public deployment still requires HTTPS, secret manager, abuse protection and security review.
P29-R06 Authenticated Personal Profile API: COMPLETED_FOR_AUTHENTICATED_PROFILE_FOUNDATION; current-user-only profile read/display-name update and personal-center overview wiring are implemented. HUMAN_REVIEW_REQUIRED. Email/phone change, verification, MFA, KYC, assets, transfers, API keys, preferences, abuse protection and production launch remain incomplete.
P29-R07 Login History Cursor Pagination: COMPLETED_FOR_AUTHENTICATED_PAGINATION_FOUNDATION; current-user-only login-history cursor pagination defaults to 10 records and supports before/after/anchor windows plus frontend bidirectional infinite scroll with URL refresh anchoring. P29-R08 extends only the current user response with IP/device snapshots. HUMAN_REVIEW_REQUIRED. It does not expose session material, manage sessions, provide MFA or production launch readiness.
P29-R08 New Device Login Verification: COMPLETED_FOR_NEW_DEVICE_VERIFICATION_FOUNDATION; trusted-device cookie, short-lived pending browser cookie, fingerprint-bound one-time email Yes/No decision and self-only successful-login IP/device snapshots are implemented. HUMAN_REVIEW_REQUIRED. Browser fingerprints are not cryptographic hardware identity; public deployment still requires TLS ingress, secret manager, CSRF/rate-abuse protection, device/session management UI, MFA/passkey, monitoring and security review.
P29-R09 Slider CAPTCHA and Registration Bloom Filter: COMPLETED_FOR_ABUSE_ADMISSION_FOUNDATION; Redis-backed server-side slider challenge, purpose/browser-fingerprint-bound single-use pass token and registration Bloom-filter precheck are implemented and Compose verified. HUMAN_REVIEW_REQUIRED. Database email uniqueness remains the final authority; CAPTCHA Redis failure is fail-closed. Rate limits, bot-detection telemetry, accessibility alternative, CSRF/TLS ingress and production security review remain incomplete.
...
資產交付計畫：已建立 `docs/planning/ASSET_RUNTIME_DELIVERY_PROGRAM.md`。ASSET-T00 為 `COMPLETED_FOR_PLANNING`；下一張為 ASSET-T01：authenticated owner scoped 的真實唯讀資產 projection/API。最終正式資產路徑不得使用 asset 或 wallet mock adapter；劃轉、入金 credit、提款、簽章與廣播仍受 ledger/reservation 與 Phase 22–25 runtime gate 阻擋。
```
