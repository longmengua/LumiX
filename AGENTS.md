...
- 第 21 階段：已規劃、尚未開工；等待人類明確命令與經審核的 task card。
...
目前 Phase 21 尚未開工。收到人類明確命令且 task card 完成審核前，Codex / mini 只能進行狀態同步與唯讀檢查：

```text
Phase 21 market data pipeline task-card definition and review
```

後續 Phase 21 的施工入口為 `docs/phases/PHASE_21_MARKET_DATA/README.md`；開始實作前必須先有經審核的 task card。
