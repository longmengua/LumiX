...
`GET /api/ops/metrics` returns an in-process snapshot for order status counts, order latency average/max, canceled order count, and emitted trade-event count. This is a lightweight baseline for local operation; production should still export metrics through a dedicated metrics backend.
