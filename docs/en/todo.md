...
- [ ] Verify timeout, retry, circuit breaker, rate limit, and idempotency coverage for every external API call.
  - Baseline done: external API inventory, hedge venue submit idempotency envelope using `refId`, CLOB place local idempotency using `clientRequestId`, CLOB cancel local replay for already-recorded cancel statuses, and approval read TTL cache coverage; CLOB sync/reconcile, cancel uncertainty handling, RPC transaction tracking, and callback effectful idempotency remain.
...
- [ ] Make CLOB place, cancel, sync, and reconcile commands idempotent.
  - Baseline done: place can use `clientRequestId`; cancel locally replays already-recorded cancel statuses. Remaining: durable command identity, uncertain cancel lookup, sync/reconcile idempotency, and full state-machine transitions.
