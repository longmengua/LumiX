...
- [ ] Build market-maker interfaces for quoting, inventory, risk limits, kill switch, and hedging order routing.
  - Baseline done: durable profile/risk-limit storage, admin profile APIs, exposure aggregation, quote command validation, stale quote cleanup, post-only quote order placement, durable active quote state/operator lookup, kill switch, slippage control, hedge venue contract, safe rejecting adapter, quote/hedge decision audit events, durable hedge decision audit trails, and hedge fill audit persistence.
