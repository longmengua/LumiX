...
- [x] Deploy WebSocket/SSE gateway independently with horizontal scaling, subscription authorization, heartbeat, rate limiting, and disconnect recovery.
  - Baseline done: gateway heartbeat contract emits `gateway.heartbeat` to SSE/WebSocket channels with channel and timestamp payload, removes closed WebSocket sessions, and has disabled-by-default scheduler config. Private user SSE/WebSocket streams require API key or Bearer credentials when `api-auth.enabled=true`; admin principals can subscribe for operations, while user principals need matching uid ownership plus stream read scope. SSE/WebSocket stream subscription attempts pass through a per-client fixed-window limiter under `push-gateway.rate-limit.*`. `push-gateway.runtime.*` exposes MONOLITH/GATEWAY role, instance id, accepting-new-streams, and draining controls; draining rejects new SSE/WebSocket streams with 503 while existing streams can drain. `GET /api/ops/push-gateway/status` exposes runtime/readiness status, and clients can use the recovery cursor plus depth/trade replay endpoints after reconnect. [Market data gateway scaling](market-data-gateway-scaling.md) documents independent gateway role, broadcast fanout, load-balancer draining, shared rate-limit options, heartbeat policy, readiness, and rollback.
...
- [x] Deploy the user WebSocket service independently with reconnect, checkpoint, event deduplication, persistence, and replay.
  - Baseline done: user WebSocket worker publishes user-channel messages to Kafka, persists a durable wallet-scoped checkpoint after publish/replay, and can replay persisted `prediction_polymarket_ws_event` rows after the checkpoint for restart recovery tests. `polymarket.ws.user-worker-role`, `user-worker-instance-id`, and `user-replay-batch-size` identify independent worker deployments; startup replays a bounded checkpoint batch before connecting when `polymarket.ws.user-enabled=true`; `GET /api/prediction/ws/user/status` exposes worker identity, stream key, checkpoint, and replay batch size; `POST /api/prediction/ws/user/replay?maxEvents=...` gives operators an explicit checkpoint replay control.
