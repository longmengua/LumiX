...
- [ ] Build market-maker hedging strategy baseline: exposure aggregation, hedge venue adapter interface, execution policy, slippage controls, and hedge audit trail.
  - Baseline done: exposure aggregation, hedge venue adapter interface, real venue signed-request skeleton, per-run execution route/notional cap policy shared across enabled-profile batches, slippage controls, durable scheduled-worker lock, operator approval token gate, bounded operator queries/ref prefixes, optional venue callback HMAC/timestamp verification, uncertain outcome venue lookup/reconcile contract, stale quote cleanup, post-only quote order placement, durable active quote state/operator lookup, per-side quote version metadata, quote/open-order reconciliation, durable hedge decision audit trail, and fill audit persistence; real venue HTTP transport/lookup, automated quote reconciliation repair jobs, and trade/ledger hedge reconciliation remain.
