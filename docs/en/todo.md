...
- [ ] Verify timeout, retry, circuit breaker, rate limit, and idempotency coverage for every external API call.
  - Baseline done: external API inventory, hedge venue submit idempotency envelope using `refId`, CLOB place local idempotency using `clientRequestId`, and CLOB cancel local replay for already-recorded cancel statuses; CLOB sync/reconcile, cancel uncertainty handling, RPC, and callback effectful idempotency remain.
