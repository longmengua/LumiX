...
- [ ] Verify timeout, retry, circuit breaker, rate limit, and idempotency coverage for every external API call.
  - Baseline done: external API inventory, hedge venue submit idempotency envelope using `refId`, and CLOB place local idempotency using `clientRequestId`; CLOB cancel/sync/reconcile, RPC, and callback effectful idempotency remain.
