...
- Reconciliation now has persisted reports, a configurable scheduler policy, alert-route baseline, event-store coverage checks, trial-balance calculation, structured ledger replay comparison, issue workflow fields, admin issue workflow APIs, workflow audit events, and a durable-ledger daily finance report baseline. Broader fee/funding/liquidation/bonus/transfer exports remain incomplete.
