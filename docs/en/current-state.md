...
- The client exchange console loads selectable markets from `/api/markets`, only offers enabled admin-configured symbols, uses the authenticated session UID instead of an editable UID field, and displays animated order-book depth plus visible market-maker quote flow.
