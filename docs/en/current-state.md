...
- The prod-facing client exchange console loads selectable markets from `/api/markets` into the order-entry form, only offers enabled admin-configured symbols, exposes no privileged admin navigation, market-maker telemetry, checksum, book-version diagnostics, or customer-facing market-data reload button, uses the authenticated session UID instead of an editable UID field, and displays animated public order-book depth with 5/10/20/50 ask and bid ticks per side; the main surface stays focused on trading, while login, registration, a single asset summary, logout, open orders, held positions, and position history live in the top-right profile drawer. Customer registration now uses a decoupled `customer_registration_requests` table for pending account material and backup link tokens, plus `customer_verification_codes` for independent six-digit email codes that can later be resent or handled by admin tooling without binding the code to one feature; `/api/auth/resend-verification` rotates a pending code/link without extending the original 24-hour request expiry. SMTP delivery, `/api/auth/verify-email`, and 24-hour pending-request expiry run before a verified request creates `app_users`/account rows; `/api/auth/config` drives Turnstile-compatible human verification. Public market and private user refresh share one `/ws/exchange` multiplex WebSocket, and polling is only a disconnect/reconnect fallback with opt-in cancel-on-disconnect resume metadata. Market-data diagnostics live in the admin market-config page.
- The client exchange console and market-maker admin page support English, Traditional Chinese, Bahasa Malaysia, and Korean locale switching with a shared browser preference.
...
- Admin market-config, risk-parameters, market-maker operations, and DLQ have static/API baselines. Market-maker operations now use a single operator-console list that keeps disabled profiles visible for recovery, expands each maker into strategy limits, auto quote policy/frequency, active quote state, maker order fills, inventory exposure/skew, hedge reconciliation/fills, idempotency status, and row-scoped guarded manual hedge actions through existing `/api/market-maker` APIs; auto quote defaults to 50 levels per side and quote order placement preflights top-of-book so maker quotes do not become taker orders. Broader reporting, load testing, rollout, and compliance features are not complete.
