...
- Polymarket CLOB place now has a `clientRequestId` local idempotency baseline, and CLOB cancel replays already-recorded cancel statuses locally; broader order lifecycle, schema versioning, sync/reconcile idempotency, cancel uncertainty handling, and the user WebSocket worker are mostly still TODO.
