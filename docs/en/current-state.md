...
| P1 Strongly Recommended | 18 | 4 | Operations, market data, Polymarket, and observability still need hardening. |
...
- Actual tracing exporter wiring, dashboards, and alert backend integration are incomplete; tracing export/sampling policy config now exists under `tracing.export.*`, and alert-rule baseline docs cover matching halt, Kafka lag, DLQ buildup, reconciliation failure, external API error rate, and unbalanced assets.
...
2. Finish the remaining P1 blockers: independently deployed WebSocket/SSE gateway, independently deployed Polymarket user WebSocket worker, tracing exporter/dashboard wiring, and alert backend integration.
