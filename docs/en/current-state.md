...
| P2 Evolution | 0 | 5 | Admin market-config, risk-parameters, and DLQ have read-only API/static page baselines; broader admin, reporting, load testing, compliance, and rollout controls remain incomplete. |
...
- The client exchange console loads selectable markets from `/api/markets`, only offers enabled admin-configured symbols, and uses the authenticated session UID instead of an editable UID field.
...
- Admin market-config, risk-parameters, and DLQ have read-only API/static page baselines with disabled write/replay actions; broader admin console, reporting, load testing, rollout, and compliance features are not complete.
