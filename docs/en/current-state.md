...
| P1 Strongly Recommended | 17 | 5 | Operations, market data, Polymarket, and data governance still need hardening. |
| P2 Evolution | 0 | 5 | Admin, reporting, load testing, compliance, and rollout controls have not started. |
| Total | 60 | 10 | The core baseline is closed, but production hardening and evolution work remain. |
...
- Polymarket CLOB place now has a `clientRequestId` local idempotency baseline, CLOB cancel can use durable `commandId` records, CLOB cancel replays already-recorded cancel/uncertain statuses locally, reconcile can resolve uncertain cancel from remote CLOB status, sync/reconcile skip unchanged local writes, a local/CLOB/trade/settlement transition matrix prevents stale active payloads from downgrading local filled/settled terminal orders or matched size and routes user-channel trade matches into the local matched lifecycle, approval reads have TTL cache coverage, session signer lifecycle guards cover expiration/revocation/abnormal-use warnings, user-channel callbacks no-op duplicate `eventKey` replays including save-race duplicates, backend-observed RPC transactions have a durable command/txHash tracking envelope with unresolved outcome reporting, and Gamma/CLOB responses now have versioned schema reports for remote-field drift; durable trade lifecycle projection, settlement downgrade tests, and the independently deployed user WebSocket worker are still TODO.
