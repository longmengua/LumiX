...
- The prod-facing client exchange console loads selectable markets from `/api/markets`, only offers enabled admin-configured symbols, exposes no privileged admin navigation, market-maker telemetry, checksum, or book-version diagnostics, uses the authenticated session UID instead of an editable UID field, and displays animated public order-book depth with 5/10/20/50 ask and bid ticks per side; public market and private user refresh share one `/ws/exchange` multiplex WebSocket, and polling is only a disconnect/reconnect fallback with opt-in cancel-on-disconnect resume metadata. Market-data diagnostics live in the admin market-config page.
