...
- The ledger now has a durable double-entry journal, bonus-credit account separation, configurable bonus consume eligibility, bonus expiry and campaign auto-clawback scheduler baselines, bonus user/campaign report and clawback APIs, turnover facts, turnover summary/drill-down queries, match-level turnover-vs-trade-tape reconciliation, disabled-by-default recent-window turnover trade/ledger-ref reconciliation, and replay path; audit retention, deeper replay validation, exportable campaign reporting, and operational controls are still incomplete.
