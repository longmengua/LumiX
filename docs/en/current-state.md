...
- The client exchange console loads selectable markets from `/api/markets`, only offers enabled admin-configured symbols, uses the authenticated session UID instead of an editable UID field, and displays animated order-book depth plus visible market-maker quote flow; public market and private user refresh share one `/ws/exchange` multiplex WebSocket with polling fallback during disconnect/reconnect windows and opt-in cancel-on-disconnect resume metadata.
