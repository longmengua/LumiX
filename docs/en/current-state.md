...
| P2 Evolution | 0 | 5 | Admin, reporting, load testing, compliance, and rollout controls are not implemented; task specs are ready under [P2 evolution tasks](../tasks/p2/README.md). |
...
- Database indexing now has a Flyway baseline for durable order lifecycle projections/events, ledger entries/postings, outbox/DLQ/matching events, and prediction order/user-event queries; live order SQL mirror design uses `order_lifecycle_projection`, live position mirror design defines future `position_lifecycle_projection`, and archive exporter skeletons exist for historical order/trade/ledger export plans. Object-storage archive writers, delete jobs, and the future position mirror schema are still incomplete.
- Operations metrics now expose in-process matching latency, rejection rate, fill rate, DB operation latency, Redis operation latency, and Kafka consumer lag counters; production metrics export is still incomplete.
...
- Metrics backend, actual tracing exporter wiring, dashboards, and alert backend integration are incomplete; tracing export/sampling policy config now exists under `tracing.export.*`, and alert-rule baseline docs cover matching halt, Kafka lag, DLQ buildup, reconciliation failure, external API error rate, and unbalanced assets.
- Admin console, reporting, load testing, rollout, and compliance features are not complete; the P2 roadmap is now split into 14 executable task specs under [P2 evolution tasks](../tasks/p2/README.md).
