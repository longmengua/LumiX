...
| --- | --- | --- | --- |
| `server/src/main/java/com/lumix/market` | `MarketDataService`、展示用 DTO 與 `DefaultMarketDataService` | `getDepth()` 以 `Instant.now()` 建立空快照；其餘查詢回傳空集合或空值；沒有 event envelope、來源、sequence、health、aggregation、stream、controller 或 provider | 此 package 是 legacy stub boundary，不能作為 authoritative market data；T02 前不得重用其 DTO 作 normalized contract |
