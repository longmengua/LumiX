...
2026-09-18 新增 `/api/admin/v1/assets/audit/adjustments` 作為受同一 server-side super-admin gate 保護的 bounded readonly 對賬視圖。它只讀取既有 `ADMIN_ASSET_ADJUSTMENT` 成功操作證據、對應 `ADJUSTMENT` journal 與 ledger entries，交叉檢查雙分錄筆數、借貸方向、同資產、借貸平衡與目標帳戶分錄。任何缺口只回傳 `EXCEPTION` 供人工調查，沒有補帳、重送、balance／ledger／audit mutation。手續費、營收、虧損與 P&L 因尚無帳務來源，明確不顯示 placeholder、零值或推估資料。`HUMAN_REVIEW_REQUIRED: yes`。
