...
| Hedge venue submit/callback | `HedgeVenueAdapter`, `MarketMakerHedgeFillService` | 外部 effectful write plus callback reconciliation | `RetryingHedgeVenueAdapter`、`ThrottlingHedgeVenueAdapter`、`RealHedgeVenueAdapter` skeleton | Submit baseline：`IdempotentHedgeVenueAdapter` 要求 `refId`，會先 claim 再送 venue、durably 保存 terminal results、拒絕 refId conflict，並在 pending/uncertain outcome 後阻止 duplicate submit。Callback baseline：`venueOrderId + venueFillId` 會 replay 既有 hedge fill audit row。Real venue baseline 先固定 signed request contract，但 HTTP I/O 仍保持 disabled/not implemented。 |
