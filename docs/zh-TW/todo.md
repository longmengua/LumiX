...
- [ ] 建立做市商 interface：報價、inventory、risk limit、kill switch 與 hedge order routing。
  - Baseline 已完成：durable profile/risk-limit storage、admin profile API、exposure aggregation、quote command validation、post-only quote order placement、kill switch、slippage control、hedge venue contract、預設安全拒絕 adapter、quote/hedge decision audit events、durable hedge decision audit trails 與 hedge fill audit persistence。
