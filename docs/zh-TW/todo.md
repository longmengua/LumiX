...
- [ ] 補流水 tracking：按 user、account、symbol、strategy、market-maker 維度統計，並能與 ledger/trade reconciliation 對齊。
  - Baseline 已完成：已處理成交會寫 durable turnover records，並有 uid/symbol/strategy/market-maker/match 維度的 turnover summary query API；reconciliation job 與正式 strategy/market-maker 下單欄位仍待補。
