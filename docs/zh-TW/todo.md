...
- [ ] 逐一確認所有外部 API 都具備 timeout、retry、circuit breaker、rate limit 與 idempotency coverage。
  - Baseline 已完成：external API inventory、使用 `refId` 的 hedge venue submit idempotency envelope、使用 `clientRequestId` 的 CLOB place local idempotency，以及 CLOB cancel 對已記錄 cancel 狀態的 local replay；CLOB sync/reconcile、cancel uncertainty handling、RPC、callback effectful idempotency 仍待補。
