...
- [ ] 逐一確認所有外部 API 都具備 timeout、retry、circuit breaker、rate limit 與 idempotency coverage。
  - Baseline 已完成：external API inventory、使用 `refId` 的 durable hedge venue submit idempotency envelope、使用 `clientRequestId` 的 CLOB place local idempotency、CLOB cancel 對已記錄 cancel/uncertain 狀態的 local replay、CLOB sync/reconcile 對未變更 payload 的 no-op local replay，以及 approval read TTL cache coverage；durable CLOB command identity、cancel uncertainty resolution、RPC transaction tracking、callback effectful idempotency 仍待補。
...
- [ ] 對 CLOB 下單、取消、同步、reconcile 做 idempotent command 設計。
  - Baseline 已完成：place 可用 `clientRequestId`；cancel 會 local replay 已記錄的 cancel/uncertain 狀態；sync/reconcile 會跳過未變更 local writes。Remaining：durable command identity、remote lookup/reconcile 解除 uncertain cancel 與完整 state-machine transitions。
