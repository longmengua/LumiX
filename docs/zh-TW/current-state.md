...
- ledger 已有 durable double-entry journal、體驗金獨立帳戶、體驗金 consume eligibility gate、體驗金到期 scanner 與 campaign auto-clawback scheduler baseline、體驗金用戶/活動 report 與 clawback APIs、流水 facts、流水 summary/drill-down queries、match-level turnover-vs-trade-tape reconciliation 與 replay path；audit retention、更深入 replay validation、可匯出活動報表、流水與 ledger 對帳與營運控制仍未完成。
