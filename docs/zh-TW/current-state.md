...
- reconciliation 已有 persisted reports、可設定排程策略、alert-route baseline、event-store coverage checks、trial-balance 計算與 daily snapshot persistence、結構化 ledger replay comparison、issue status/owner/resolved_at workflow 欄位、後台 issue workflow API、workflow audit events，以及 durable-ledger daily finance report baseline；更完整 fee/funding/liquidation/bonus/transfer 匯出仍未完成。
