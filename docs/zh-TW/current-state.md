...
- 前台 exchange console 會從 `/api/markets` 載入可選市場，只提供後台配置且啟用的 symbols，並以登入 session UID 取代可編輯 UID 欄位，也會顯示動態 order-book 深度條與客戶端可見的做市商 quote flow；depth、做市商 flow、open orders 目前以 1 秒 polling 更新，尚未改成 WebSocket push。
