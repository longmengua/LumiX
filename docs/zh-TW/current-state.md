...
| P2 演進項 | 0 | 5 | Admin market-config 已有 read-only API / 靜態頁 baseline；更完整的後台、報表、壓測、合規與灰度功能仍未完成。 |
...
- Admin market-config 已有 read-only API / 靜態頁 baseline，write actions 仍停用；更完整的 admin console、報表、壓測、灰度與合規功能都還沒完成。
