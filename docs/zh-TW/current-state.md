...
- Polymarket CLOB place 已有 `clientRequestId` local idempotency baseline，CLOB cancel 也會對已記錄的 cancel 狀態做 local replay，approval reads 已有 TTL cache coverage；更完整的 order lifecycle、schema versioning、sync/reconcile idempotency、cancel uncertainty handling、approval transaction tracking、user WebSocket worker 還大多是待辦。
