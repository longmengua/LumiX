...
- Polymarket CLOB place 已有 `clientRequestId` local idempotency baseline，CLOB cancel 也會對已記錄的 cancel 狀態做 local replay；更完整的 order lifecycle、schema versioning、sync/reconcile idempotency、cancel uncertainty handling、user WebSocket worker 還大多是待辦。
