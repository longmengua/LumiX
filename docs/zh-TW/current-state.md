...
- 體驗金已用 `USER_BONUS_AVAILABLE` 拆出 grant、consume、expire、clawback ledger postings，且不會改動真實現金帳戶餘額；目前也有 grant 批次 expiry/remaining tracking、可設定 consume eligibility gate、用戶/活動 report APIs 與營運 clawback API baseline。
...
- ledger 已有 durable double-entry journal、體驗金獨立帳戶、體驗金 consume eligibility gate、體驗金到期 scanner baseline、體驗金用戶/活動 report 與 clawback APIs、流水 facts、流水 summary/drill-down queries 與 replay path；audit retention、更深入 replay validation、可匯出活動報表、流水對帳與營運控制仍未完成。
