<!-- 檔案用途：歷史訂單、成交、ledger、Kafka event、audit log 的 production archive strategy；英文版位於 ../en/archive-strategy.md。 -->
# Archive Strategy

這份策略定義哪些資料可以離開 hot table 或 Redis、哪些資料必須保留查詢能力，以及 archived data restore 時以哪個來源為準。

English version: [../en/archive-strategy.md](../en/archive-strategy.md)
...
- 為每個 data family 實作 archive exporter jobs 與 manifests。
