<!-- 檔案用途：繁體中文 core-v1 release freeze checklist。英文版：../en/core-v1-release-checklist.md。 -->
# Core V1 Release Checklist

這份清單用來把目前 production-core baseline 收斂成有邊界的 `core-v1` release candidate。它不代表系統已可承載真實資金 production。

英文版：[../en/core-v1-release-checklist.md](../en/core-v1-release-checklist.md)
...
- [ ] `./mvnw test` 通過。
- [ ] `git diff --check` 通過。
- [ ] Flyway migrations append-only 且版本順序正確。
- [ ] 會改動狀態的 scheduler 預設關閉，除非明確要啟用。
- [ ] Protected admin APIs 已被 `/api/market-maker/**`、`/api/recovery/**`、`/api/risk/**` 與相關 security classifier paths 覆蓋。
- [ ] current-state 與 TODO 文件指向 freeze task，而不是繼續擴功能。
- [ ] 已知 production blockers 有寫清楚，沒有被默認成已完成。
