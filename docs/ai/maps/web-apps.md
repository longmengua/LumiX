...
- `exchange.html` is the client exchange console for first-party auth, admin-configured market selection, depth, order entry, open orders, and account lookup through `/api/auth`, `/api/markets`, `/api/depth`, `/api/order`, and `/api/margin`; client UID is derived from the authenticated session rather than editable form input.
