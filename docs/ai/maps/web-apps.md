...
- `exchange.html`, `admin-console.html`, and `admin-market-maker.html` share the `localStorage.exchangeLanguage` locale preference for English, Traditional Chinese, Bahasa Malaysia, and Korean; English remains the default for first-load demos and browser tests.
