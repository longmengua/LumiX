...
- Admin UI: `src/main/resources/static/admin-market-maker.html` uses a single operator-console list; disabled profiles remain visible so operators can re-enable them, and each expanded market-maker row owns its strategy limits, auto quote policy/frequency, active quotes, hedge fills/reconciliation, idempotency status, and guarded manual hedge actions without raw response debug panels.
