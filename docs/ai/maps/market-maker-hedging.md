...
- Auto quote baseline: `MarketMakerAutoQuoteService` derives a configurable bid/ask ladder from current two-sided top-of-book midpoint, symbol tick/lot/minNotional, per-symbol risk limits, and a small configurable tick pulse for demos; `MarketMakerAutoQuoteRunner` is default-disabled by `market-maker.auto-quote.enabled=false`, defaults to 50 levels per side through `market-maker.auto-quote.ladder-levels-per-side`, and calls the existing quote lifecycle ladder path so replacement, state persistence, and `market-maker.quote` WebSocket pushes stay on the same path as manual quote commands while old quote orders are canceled only once per ladder refresh.
