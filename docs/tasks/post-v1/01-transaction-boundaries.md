...
- Added tests proving successful command bodies commit, failed command bodies roll back without hidden retry, and active transactions do not send outbox payloads before commit.

Remaining work:
- Extend the same boundary pattern to liquidation, ADL execution, and hedge execution command paths.
- Add persistence-backed integration tests that prove DB state and outbox rows roll back together under MySQL.
- Document Redis hot-state recovery rules for cases where DB commits but cache writes fail.
