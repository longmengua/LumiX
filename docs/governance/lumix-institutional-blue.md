...
狀態：正式設計治理。適用管理後台；建立於 2026-09-17。本文是視覺設計政策的唯一權威來源，不代表目前所有頁面皆已符合規範。實際元件、頁面成熟度、已知差距與同步紀錄統一登錄於 [LumiX UI 長期治理與實作同步文件](../ai/LUMIX_UI_GOVERNANCE.md)，避免本文重複保存容易過期的 implementation snapshot。
