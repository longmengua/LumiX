...
| ASSET-T11 | 新註冊使用者帳戶容器 provisioning | P13 identity/account、P29 registration | `COMPLETED_FOR_ACCOUNT_BOUNDARY` | 每位已驗證新使用者原子建立 SPOT／FUTURES 空帳戶；不建立餘額或 ledger entry |
...
| 2026-09-15 | ASSET-T00 follow-up | `COMPLETED_FOR_FORMAL_ROUTE_ISOLATION` | `/assets/transfer`、入金、提款、地址簿與其歷史 route 已改為受控 unavailable state，正式 browser route 不再載入 asset/wallet mock adapter 或顯示可執行資產操作。 | 前端 typecheck/build 與 route import 檢查 |
| 2026-09-15 | ASSET-T11 | `COMPLETED_FOR_ACCOUNT_BOUNDARY` | email 雙驗證完成後的同一 primary transaction 現在會建立固定識別的 SPOT／FUTURES `accounts` rows；它們只是零餘額帳戶容器，沒有 `balance_projections`、ledger entry、資金 mutation 或 mock。 | 後端 compile；註冊 transaction rollback 仍由既有 `@Transactional` 邊界保護 |
...
| 2026-09-18 | 管理端登入／提幣凍結 | `COMPLETED_FOR_GOVERNED_RESTRICTION_RUNTIME` | `PUT /api/admin/v1/users/{userId}/restrictions/login` 與 `/withdrawal` 均只接受已啟用最高管理員 session。登入凍結使用既有 `SUSPENDED` 狀態並立即撤銷所有有效 session；提幣凍結以 `V025` 的 `withdrawal_frozen_at` 保存。每一有效變更在同一 transaction 寫入 immutable audit；相同目標狀態安全回應而不重複 mutation。平台內部轉出會同時檢查管理端提幣凍結與既有裝置更換冷卻限制，任一存在即 fail closed；同一使用者現貨／合約劃轉不受此限制影響。沒有錢包、私鑰、鏈上 provider、簽章或廣播。 | 後端 compile、前端 typecheck 與 migration review；`HUMAN_REVIEW_REQUIRED` |

### ASSET-T11 migration 與 rollback

`V018__provision_default_user_accounts.sql` 是歷史 migration，曾對既有 `users` 補齊帳戶容器。現行產品僅保留 SPOT／FUTURES；`V023__remove_spot_margin_accounts.sql` 會在沒有任何帳本、投影、保留、訂單或錢包證據時刪除舊有 MARGIN 容器，否則 fail-closed 中止 migration，絕不刪除稽核證據。新註冊流程只建立 SPOT／FUTURES。
