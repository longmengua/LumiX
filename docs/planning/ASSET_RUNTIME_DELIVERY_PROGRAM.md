...
| ASSET-T02 | 前台資產總覽、現貨／合約／槓桿頁切換至 ASSET-T01 API | ASSET-T01、P30 trusted UX | `IN_PROGRESS` | 移除正式頁的 asset mock adapter；完整 loading/empty/error/not-found state；無假資料 fallback |
...
| 2026-09-15 | ASSET-T02 | `IN_PROGRESS` | 已新增 browser 到 `GET /api/v1/assets/balances` 的 authenticated read adapter、response contract validation、取消舊 request 的 hook 與不經 JavaScript number 的 decimal formatter；並開始將資產表格改為只顯示 API 已提供的 available / locked / total / freshness。尚未完成總覽、現貨、合約、槓桿頁接線，未完成 typecheck/build/browser 驗證；現有正式頁仍不可宣稱已移除 mock。 | 已完成程式碼 review；等待下一次施工完成 UI 接線後再執行完整前端驗證 |

## 目前下一張 task

`ASSET-T02`：`IN_PROGRESS`。前台資產總覽、現貨／合約／槓桿頁需切換至 `GET /api/v1/assets/balances`，移除正式 route 對 `mockAssetService` 的依賴。下一次施工必須先完成頁面接線、source/freshness/empty/error/unavailable presentation、typecheck/build/browser 驗證，才可更新為完成；不得把 read model 誤稱為 ledger source of truth。
