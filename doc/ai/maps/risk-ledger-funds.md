...
- Migrations: `V20260625_init.sql.sql` contains the squashed core-v1 ledger and funds schema.
...
- `AccountPositionConsistencyService` validates restored accounts against open positions and reports missing accounts or account margin below open-position margin through `/api/recovery/restore/account-position-consistency`.
- Transaction-boundary tests cover rollback semantics for order-place outbox insert failure, cancel ledger-release failure, cancel-replace reserve-release/replacement reserve failure, and hedge audit/outbox persistence failure.
