...
- Admin UI: `src/main/resources/static/admin-market-maker.html` uses a single operator-console list; disabled profiles remain visible so operators can re-enable them, and each expanded market-maker row owns its strategy limits, auto quote policy/frequency, active quotes, maker order fills, inventory exposure/skew, hedge fills/reconciliation, idempotency status, and guarded manual hedge actions without raw response debug panels.
...
- Hedge fill/reconciliation admin queries: `/api/market-maker/profiles/{marketMakerId}/exposures`, `/api/market-maker/profiles/{marketMakerId}/hedge-fills`, `/api/market-maker/hedge-fills/venue-orders/{venueOrderId}`, `/api/market-maker/hedge-fills/ref/{refId}`, `/api/market-maker/profiles/{marketMakerId}/hedge-reconciliation`, `/api/market-maker/hedge-idempotency/unresolved`, `POST /api/market-maker/hedge-idempotency/reconcile`.
...
- Quote validation and placement lifecycle: `MarketMakerQuoteService`, `MarketMakerQuoteLifecycleService`, `MarketMakerQuoteOrderGateway`, `UseCaseMarketMakerQuoteOrderGateway`; accepted or rejected quotes first clear prior open `mmq:{marketMakerId}:...` quote orders for the same uid/symbol before replacement placement, preflight post-only legs against top-of-book so maker quotes cannot become taker orders, then persist latest active quote state for restart/operator lookup.
- Auto quote baseline: `MarketMakerAutoQuoteService` is a safe-market-making baseline that prefers latest ticker fair value, falls back to internal two-sided top-of-book, widens half-spread by effective maker fee, hedge taker fee, and `market-maker.auto-quote.hedge-cost-rate`, then applies configurable tick ladder/pulse inside that safety envelope; `MarketMakerAutoQuoteRunner` is default-disabled by `market-maker.auto-quote.enabled=false`, defaults to 50 levels per side through `market-maker.auto-quote.ladder-levels-per-side`, and calls the existing quote lifecycle ladder path so replacement, state persistence, and `market-maker.quote` WebSocket pushes stay on the same path as manual quote commands while old quote orders are canceled only once per ladder refresh.
...
- Auto quote operator APIs: `GET /api/market-maker/auto-quote/status` exposes configured runner state, and `POST /api/market-maker/auto-quote/run-once` executes one diagnostic strategy loop without requiring the continuous runner to be enabled.
...
- Auto quote runner config: `MARKET_MAKER_AUTO_QUOTE_ENABLED`, `MARKET_MAKER_AUTO_QUOTE_FIXED_DELAY_MS`, `MARKET_MAKER_AUTO_QUOTE_MAX_PROFILES_PER_RUN`, `MARKET_MAKER_AUTO_QUOTE_QUOTE_QUANTITY`, `MARKET_MAKER_AUTO_QUOTE_HALF_SPREAD_TICKS`, `MARKET_MAKER_AUTO_QUOTE_HEDGE_COST_RATE`, `MARKET_MAKER_AUTO_QUOTE_LADDER_LEVELS_PER_SIDE`, `MARKET_MAKER_AUTO_QUOTE_PULSE_TICKS`, and `MARKET_MAKER_AUTO_QUOTE_REF_PREFIX`.
...
- Tests: `MarketMakerHedgingServiceTest`, `MarketMakerQuoteServiceTest`, `MarketMakerQuoteLifecycleServiceTest`, `MarketMakerAutoQuoteServiceTest`, `MarketMakerQuoteReconciliationServiceTest`, `MarketMakerQuoteStateRecordTest`, `UseCaseMarketMakerQuoteOrderGatewayTest`, `MarketMakerProfileServiceTest`, `MarketMakerHedgeFillServiceTest`, `HedgeVenueCallbackVerifierTest`, `MarketMakerHedgeReconciliationServiceTest`, `MarketMakerHedgeVenueIdempotencyServiceTest`, `MarketMakerHedgeStrategyServiceTest`, `MarketMakerHedgeExecutionServiceTest`, `MarketMakerQuoteRateLimiterTest`, `MarketMakerHedgeExecutionRateLimiterTest`, `MarketMakerEndpointAuditLoggerTest`, `RealHedgeVenueAdapterTest`, `IdempotentHedgeVenueAdapterTest`, `RetryingHedgeVenueAdapterTest`, `ThrottlingHedgeVenueAdapterTest`, `ApiAuthenticationInterceptorTest`.
