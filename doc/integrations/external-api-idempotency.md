<!-- 檔案用途：外部 API retry / idempotency inventory。 -->
...
| Polymarket relayer/RPC approval | `PolymarketSessionService` / `PolymarketApprovalService` / `RpcTransactionTrackingService` / Web3j config | 外部 effectful writes and reads | 部分共用 config；RPC-specific limits 仍待補 | Approval reads 已有 TTL cache 與 owner-scoped clear。Backend-observed effectful RPC flow 可使用 durable `commandId` + `txHash` tracking 與 unresolved outcome report。 |
| Hedge venue submit/callback | `HedgeVenueAdapter`, `MarketMakerHedgeFillService` | 外部 effectful write plus callback reconciliation | `RetryingHedgeVenueAdapter`、`ThrottlingHedgeVenueAdapter`、`RealHedgeVenueAdapter` HTTP transport | Submit baseline：`IdempotentHedgeVenueAdapter` 要求 `refId`，會先 claim 再送 venue、durably 保存 terminal results、拒絕 refId conflict，並在 pending/uncertain outcome 後阻止 duplicate submit。Callback baseline：`venueOrderId + venueFillId` 會 replay 既有 hedge fill audit row。Real venue baseline 會簽署 submit/lookup request，透過共用 OkHttp 送出，並映射 accepted/rejected/retryable venue response。 |
...
營運可透過 `GET /api/market-maker/hedge-idempotency/unresolved` 檢視未解的 hedge venue idempotency outcomes。報告會列出 pending claims 與 completed retryable outcomes，但不暴露已保存的 payload fingerprint。`POST /api/market-maker/hedge-idempotency/reconcile` 會要求目前設定的 `HedgeVenueOrderLookupAdapter` 依 `refId` 解析 outcome；預設 adapter 是安全 no-op，直到真實 venue lookup 接上。
...
- Stale active CLOB payload 不會降級 local filled/settled terminal order 或 matched size。
- 更完整的 local/CLOB/trade/settlement event payload transitions 仍待補。
...
Backend-observed effectful RPC transaction 會透過 `RpcTransactionTrackingService` 建立 durable tracking envelope，供未來 relayer 或 backend submit path 在把 tx 視為已送出前先落 idempotency record。

- `commandId` 是 effectful RPC command 的 idempotency key。
- `fingerprint` 描述預期鏈上效果，例如 approval 類流程的 owner/spender/token/amount。
- 相同 `commandId`、`fingerprint` 與 `txHash` 會 replay 既有 transaction record。
- 相同 `commandId` 但 `fingerprint` 或 `txHash` 不同時，會拒絕為 idempotency conflict。
- `V10__rpc_transaction_records.sql` 會保存 command、chain、wallet、transaction type、fingerprint、transaction hash、status 與 completion state。
- 營運可用 `GET /api/prediction/approve/rpc-transactions/unresolved?limit=100` 查詢未解 submitted transactions。
