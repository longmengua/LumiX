...
- `exchange.html`：prod-facing 前台交易操作台，覆蓋註冊/登入/登出、order book、下單、掛單、帳戶查詢；market selector 位於下單區並必須從 `/api/markets` 載入後台配置出的可交易市場，不讓使用者自由輸入未配置幣種，也不暴露任何後台導覽、做市商營運資訊、checksum 或 book version。前台主畫面只保留 Trade 分頁，維持 `Order Book` / `Place Order` 同一 row、下方交易資訊區以 `倉位` / `委託` / `歷史交易資訊` 三個 tab 呈現；登入、註冊、資產摘要與登出全部集中在右上 profile drawer，未登入時顯示 email/password 登入卡片、主要登入 CTA 與次要註冊入口，並可依 `/api/auth/config` 顯示 Cloudflare Turnstile 相容免費真人驗證；註冊只建立 pending request，主流程是在畫面輸入六位數 email code 完成驗證，郵件 link 只作備案，不顯示成主要 CTA，登入後顯示單一資產摘要，並在個人頁以 `倉位` / `委託` / `歷史交易資訊` 三個 tab 呈現交易資料；UID 只能由登入狀態帶出，不做可輸入欄位，帳戶資料由 session/auth refresh 更新，不提供手動帳戶 reload、市場資訊 reload 或 raw auth/account debug log；委託 refresh 使用 icon-only button 並保留多語系 accessibility label。Order book 只顯示一般市場深度、spread 與動態深度條，不標記做市商身份；前台使用單一 `/ws/exchange` multiplex WebSocket，未登入先送 `subscribe.market`，登入後同線路再送 `subscribe.user`，用公開 market / order lifecycle / trade signal 更新 depth 與委託，斷線或重連前才退回 1 秒 polling；`subscribe.user` 支援 opt-in `cancelOnDisconnect` 與 `resumeConnectionId`。
- `css/exchange.css`：前台 exchange console 的樣式模組。
- `js/exchange.js`：前台 exchange console 的瀏覽器互動、API 呼叫、WebSocket 與 i18n 模組。
- `exchange-admin.html`：後台統一入口，以 tabs 集中測試金、market config、risk parameters、market makers、DLQ；做市商後台以 embedded tab 顯示在同一個後台畫面；後台外殼支援四語系並同步語言偏好到同源子頁。
...
- `admin-market-maker.html`：交易所做市後台 profile / strategy / hedge operation 內容頁，搭配 `/api/market-maker` profile、quote、hedge execution、reconciliation APIs；從 `exchange-admin.html` 以 `?embed=1` 開啟時會隱藏內層 header，避免像另一個後台畫面。
- `exchange.html`、`exchange-admin.html` 與 `admin-market-maker.html`：支援 English、中文、Bahasa Malaysia、한국어 語言切換，偏好保存在 `localStorage.exchangeLanguage`；前台交易頁是 prod-facing 客戶入口，不暴露後台導覽。
