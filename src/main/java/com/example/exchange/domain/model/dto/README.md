...
- Polymarket：Gamma/CLOB/user WS 相關 request/response DTO；`PolymarketPlaceOrderRequest.clientRequestId` 是 CLOB place idempotency key，`PolymarketClobCommandRecord` 是 effectful CLOB command idempotency record，`PolymarketResponseSchemaReport` 是外部 Gamma/CLOB response schema drift report。
