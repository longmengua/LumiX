...
- `MatchingBookRecoveryService` / `MatchingBookRecoveryStartupListener`：REST-mode MVP 啟動時重建本機 in-memory order book；先跑 durable recovery，若 book 仍空可用 persisted open orders fallback，避免 depth 與 Redis open orders 分裂。
