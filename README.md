...
- 行情/私有推送：`GET /api/market-data/{symbol}/stream`、`GET /api/market-data/user/{uid}/stream`、`WS /ws/market/{symbol}`、`WS /ws/user/{uid}`
- 資金費/強平：`POST /api/risk/funding/settle`、`POST /api/risk/liquidate`
- 恢復/校驗：`POST /api/recovery/recover/{uid}?fromSeq=...`、`GET /api/recovery/validate/{uid}`
...
- funding / liquidation 已具備 MVP 帳務與事件；正式環境仍需接標記價/指數價 oracle 與自動化風控觸發。
- WebSocket / SSE gateway 已接 market data service；正式部署需加 auth、節流與 gateway 水平擴展。
...
P0 目前是可測 MVP：已具備資料表、API、Kafka consumer、WS event 冪等、CLOB 查詢/取消與本地 order reconciliation。P1/P2 已補內部交易所核心的 ledger、風控、sequencer、行情推送、funding、liquidation、outbox/DLQ、snapshot replay 與 migration 基線；觀測、壓測與安全合規仍列在後續 P2。
...
- [x] 補 liquidation engine：部分平倉、全平、保險基金、ADL。
- [x] 補 funding rate 計算與結算流水。
...
- [x] Kafka topic 按事件類型與 symbol partition 規劃，確保同 symbol 順序。
- [x] Market data service 輸出 depth delta、ticker、trade tape、kline。
- [x] WebSocket gateway 支援訂單簿與使用者私有推送。

### P2：可靠性與資料恢復

- [x] Snapshot + event replay 完整化，支援指定 offset 恢復。
- [x] 所有關鍵事件加 idempotency key。
- [x] 補 outbox pattern，避免 DB commit 成功但 Kafka publish 失敗。
- [x] 補 DLQ、retry、poison message 處理。
- [x] 補資料校驗 job：order/trade/position/ledger 對帳。
- [x] 補 Flyway 或 Liquibase 管理 schema migration，移除正式環境 `ddl-auto=update`。
