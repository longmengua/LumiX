<!-- 檔案用途：專案入口文件，說明 java21-match-hub 的業務功能、模塊、架構與本機啟動方式。 -->
# Java21 Match Hub
...
目前內部交易所下單入口是 `POST /api/order/place`，同步完成基本驗證、風控預檢、撮合、帳務與事件發布後回傳 `accepted`。這條鏈路仍是 MVP 實作，production 前建議參考 `todo_ch.md` 補齊持久化撮合、完整 order lifecycle 與更嚴格的帳務一致性。
