...
type AdjustmentType = 'activity' | 'airdrop' | 'reversal';

const SECTION_PATHS: Record<AssetSection, string> = {
  users: '/assets/users',
  adjustments: '/assets/adjustments/reversal',
  audit: '/assets/audit',
};
...
export function AdminAssetsPage() {
  ...
  const adjustmentType = adjustmentTypeFromPath(normalizedPath);
  ...
  if (normalizedPath.endsWith('/compensation')) return <Navigate replace to="/assets/adjustments/reversal" />;
    ...
    <div className="stack">
      ...
      <div className="admin-assets-workspace">
        ...
        <div className="admin-assets-workspace__content">
          ...
          {section === 'adjustments' ? <AdjustmentWorkspace activeType={adjustmentType} onSelect={(type) => navigate(`/assets/adjustments/${type}`)} /> : null}
        </div>
      </div>
    </div>
  );
...
function adjustmentTypeFromPath(pathname: string): AdjustmentType {
  if (pathname.endsWith('/activity')) return 'activity';
  if (pathname.endsWith('/airdrop')) return 'airdrop';
  if (pathname.endsWith('/reversal')) return 'reversal';
  return 'reversal';
}
