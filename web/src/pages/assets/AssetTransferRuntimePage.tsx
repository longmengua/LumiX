...
export function AssetTransferRuntimePage() {
  ...
  return <div className="stack assets-page"><PageHeader title={t('assets.transferTitle')} description={t('assets.transferRuntimeDescription')} /><AssetSectionNav />
    <Card title={t('assets.transferTitle')}><form className="admin-assets-transfer" onSubmit={(event) => void submit(event)}>
      <label className="field"><span className="field__label">{t('assets.transferRuntimeFrom')}<span className="admin-assets-transfer__required" aria-hidden="true">*</span></span><select className="input" required value={form.sourceAccountType} onChange={(event) => update('sourceAccountType', event.target.value)}><option value="SPOT">{t('account.spotAccount')}</option><option value="FUTURES">{t('account.futuresAccount')}</option></select></label>
      <label className="field"><span className="field__label">{t('assets.transferRuntimeTo')}<span className="admin-assets-transfer__required" aria-hidden="true">*</span></span><select className="input" required value={form.destinationAccountType} onChange={(event) => update('destinationAccountType', event.target.value)}><option value="SPOT">{t('account.spotAccount')}</option><option value="FUTURES">{t('account.futuresAccount')}</option></select></label>
