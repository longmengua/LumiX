...
export function AssetTransferRuntimePage() {
  ...
  return <div className="stack assets-page"><PageHeader title={t('assets.transferTitle')} description={t('assets.transferRuntimeDescription')} /><AssetSectionNav />
    <Card title={t('assets.transferTitle')}><form className="admin-assets-airdrop" onSubmit={(event) => void submit(event)}>
      <label className="field"><span className="field__label">{t('assets.transferRuntimeFrom')}</span><select className="input" value={form.sourceAccountType} onChange={(event) => update('sourceAccountType', event.target.value)}><option>SPOT</option><option>FUTURES</option><option>MARGIN</option></select></label>
      <label className="field"><span className="field__label">{t('assets.transferRuntimeTo')}</span><select className="input" value={form.destinationAccountType} onChange={(event) => update('destinationAccountType', event.target.value)}><option>SPOT</option><option>FUTURES</option><option>MARGIN</option></select></label>
